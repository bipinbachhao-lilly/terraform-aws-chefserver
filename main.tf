...
resource "aws_security_group" "ChefServerSecurityGroup" {
  vpc_id      = data.aws_vpc.selected.id
  name        = "ChefServerSecurityGroup"
...
resource "aws_instance" "ChefServerInstance" {
  ...
  vpc_security_group_ids = ["${aws_security_group.ChefServerSecurityGroup.id}"]
  /*iam_instance_profile   = aws_iam_instance_profile.nginx.id*/
  subnet_id              = tolist(data.aws_subnet_ids.public.ids)[0]
  key_name               = var.key_name
  user_data              = templatefile("${path.module}/cloudinit_conf.tpl", { workspaces = var.workspaces })
  tags = merge(
